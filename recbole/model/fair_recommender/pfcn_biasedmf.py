...
class PFCN_BiasedMF(FairRecommender):
    ...
    def calculate_loss(self, interaction, sst_list):
        ...
        user_bias = self.user_bias(user)
        pos_item_bias = self.item_bias(pos_item)
        neg_item_bias = self.item_bias(neg_item)
        global_bias = self.global_bias

        pos_scores = torch.mul(user_embed, pos_item_embed).sum(dim=-1) + user_bias + pos_item_bias + global_bias
        neg_scores = torch.mul(user_embed, neg_item_embed).sum(dim=-1) + user_bias + neg_item_bias + global_bias
